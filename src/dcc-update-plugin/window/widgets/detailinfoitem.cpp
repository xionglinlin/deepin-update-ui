...
UpdateDetailInfoItem::UpdateDetailInfoItem(QWidget *parent)
    : SettingsItem(parent)
...
void UpdateDetailInfoItem::initUi()
{
    ...
    m_titleLayout->addWidget(m_title, 0, Qt::AlignLeft | Qt::AlignTop);
    m_titleLayout->addWidget(m_dateLabel, 0, Qt::AlignRight | Qt::AlignTop);
    ...
    m_mainLayout->addLayout(bomBoxLayout);

    setLayout(m_mainLayout);
}

void UpdateDetailInfoItem::setDate(QString date)
{
    if (date.isEmpty()) {
...
void UpdateDetailInfoItem::setTitle(QString title)
{
    m_title->clear();
    m_title->setText(title);
}

void UpdateDetailInfoItem::setLinkData(QString data)
{
    if (data.isEmpty()) {
...
void UpdateDetailInfoItem::setDetailData(QString data)
{
    if (data.isEmpty()) {
...
void UpdateDetailInfoItem::setIsLastItem(bool isLastItem)
{
    // 最后一个详情才显示收起按钮
    if (isLastItem) {
        m_hideButton = new DCommandLinkButton(tr("Collapse"), this);
        connect(m_hideButton, &DCommandLinkButton::clicked, this, &UpdateDetailInfoItem::requestHideDetails);
        DFontSizeManager::instance()->bind(m_hideButton, DFontSizeManager::T8);
        m_hideButton->setForegroundRole(DPalette::Button);
        auto hideButtonLayout = new QHBoxLayout;
        hideButtonLayout->addStretch(1);
        hideButtonLayout->addWidget(m_hideButton);
        auto mainLayout = qobject_cast<QVBoxLayout *>(layout());
        if (mainLayout)
            mainLayout->addLayout(hideButtonLayout);
    }
}

void UpdateDetailInfoItem::setUpdateType(UpdateType type)
{
    if (type == UpdateType::SecurityUpdate) {
        DFontSizeManager::instance()->bind(m_title, DFontSizeManager::T7, QFont::Normal);
